...
namespace lar_pandora {
  ...
  LArPandoraGeometry::LoadDetectorGaps(LArDetectorGapList& listOfGaps)
  {
    ...
    for (LArDriftVolumeList::const_iterator iter1 = driftVolumeList.begin(),
      ...
      if (isDualPhase)
      {
          for (LArDaughterDriftVolumeList::const_iterator iterDghtr1 = driftVolume1.GetTpcVolumeList().begin(), iterDghtrEnd1 = driftVolume1.GetTpcVolumeList().end(); iterDghtr1 != iterDghtrEnd1; ++iterDghtr1)
          {
              const LArDaughterDriftVolume &tpcVolume1 = *iterDghtr1;

              for (LArDaughterDriftVolumeList::const_iterator iterDghtr2 = iterDghtr1, iterDghtrEnd2 = driftVolume1.GetTpcVolumeList().end(); iterDghtr2 != iterDghtrEnd2; ++iterDghtr2)
              {
    ...
    for (unsigned int icstat = 0; icstat < theGeometry->Ncryostats(); ++icstat) {
      ...
      for (unsigned int itpc1 = 0; itpc1 < theGeometry->NTPC(icstat); ++itpc1) {
        ...
        tpcVolumeList.emplace_back(LArDaughterDriftVolume(icstat, itpc1,
                                                          0.5f * (driftMaxX + driftMinX), 0.5f * (driftMaxY + driftMinY),
                                                          0.5f * (driftMaxZ + driftMinZ),
                                                          (driftMaxX - driftMinX), (driftMaxY - driftMinY),
                                                          (driftMaxZ - driftMinZ)));
